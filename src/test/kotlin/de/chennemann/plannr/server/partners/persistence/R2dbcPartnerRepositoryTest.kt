...
class R2dbcPartnerRepositoryTest : ApiIntegrationTest() {
    ...
    fun `finds all partners ordered by created at and id with filters`() = runBlocking {
        partnerRepository.save(PartnerFixtures.partner(id = "par_2", name = "Beta GmbH", createdAt = 2))
        partnerRepository.save(PartnerFixtures.partner(id = "par_1", name = "ACME Corp", createdAt = 1, isArchived = true))
        partnerRepository.save(PartnerFixtures.partner(id = "par_3", name = "Acme Services", createdAt = 3))
