<script lang="ts">
  import type { PocketWithContract, ContractForm, Partner, Pocket, Account } from '$lib/types';
  import { createEventDispatcher } from 'svelte';

  export let form: ContractForm;
  export let contracts: PocketWithContract[] = [];
  ...
  const dispatch = createEventDispatcher<{ submit: void; reset: void; edit: { contract: PocketWithContract }; archive: { contract: PocketWithContract }; addRecurring: { contract: PocketWithContract } }>();
...
<section class="workspace-grid">
  <article class="panel form-panel">
    ...
    <form class="form-grid" on:submit|preventDefault={() => dispatch('submit')}>
      ...
      <label><span>Signing date</span><input bind:value={form.signingDate} type="date" /></label>
      <label><span>Expiration date</span><input bind:value={form.expirationDate} type="date" /></label>
      <label><span>Last cancellation date</span><input bind:value={form.lastCancellationDate} type="date" /></label>
