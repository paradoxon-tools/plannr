...
class ArchiveContractTest {
    ...
    fun `archives contract`() = runTest {
        ...
        val archiveContract = ArchiveContractUseCase(repository, archiveCascade)
        ...
        assertEquals(listOf(ContractFixtures.DEFAULT_ID), archiveCascade.archivedContracts)
    }

    @Test
    fun `fails for unknown contract`() = runTest {
        val archiveContract = ArchiveContractUseCase(InMemoryContractRepository(), RecordingContractRecurringTransactionCascade())
