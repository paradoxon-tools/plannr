...
private data class MaterializationRecurrencePattern(
...
class RecurrenceCalculator {
    fun occurrences(
        ...
        limit: Int? = null,
        endInclusive: LocalDate? = pattern.finalOccurrenceDate?.let(LocalDate::parse),
    ): List<LocalDate> =
        occurrences(pattern.toMaterializationPattern(), limit, endInclusive)

    private fun occurrences(
        pattern: MaterializationRecurrencePattern,
        limit: Int? = null,
        endInclusive: LocalDate? = pattern.finalOccurrenceDate,
    ): List<LocalDate> {
        if (pattern.skipCount < 0) {
            throw ValidationException("validation_error", "Transaction template skip count must not be negative")
        }
    ...
    fun normalizeFinalOccurrenceDate(pattern: RecurrencePattern, maxRecurrenceCount: Int?): LocalDate? {
        val materializationPattern = pattern.toMaterializationPattern()
        val normalizedCount = maxRecurrenceCount ?: return materializationPattern.finalOccurrenceDate
        if (normalizedCount <= 0) {
            throw ValidationException("validation_error", "maxRecurrenceCount must be greater than 0")
        }
        val explicitEnd = materializationPattern.finalOccurrenceDate
        val byCount = occurrences(materializationPattern.copy(finalOccurrenceDate = null), limit = normalizedCount).lastOrNull()
        return listOfNotNull(explicitEnd, byCount).minOrNull()
    }
}

private fun RecurrencePattern.toMaterializationPattern(): MaterializationRecurrencePattern =
    MaterializationRecurrencePattern(
...
private interface RecurrenceRule {
    fun occurrences(pattern: MaterializationRecurrencePattern, endInclusive: LocalDate?): List<LocalDate>
}

private class NoRecurrence : RecurrenceRule {
    override fun occurrences(pattern: MaterializationRecurrencePattern, endInclusive: LocalDate?): List<LocalDate> =
        if (endInclusive != null && pattern.firstOccurrenceDate.isAfter(endInclusive)) emptyList() else listOf(pattern.firstOccurrenceDate)
}

private class DailyRecurrence : RecurrenceRule {
    override fun occurrences(pattern: MaterializationRecurrencePattern, endInclusive: LocalDate?): List<LocalDate> {
...
private class WeeklyRecurrence : RecurrenceRule {
    override fun occurrences(pattern: MaterializationRecurrencePattern, endInclusive: LocalDate?): List<LocalDate> {
        val result = mutableListOf<LocalDate>()
...
private class MonthlyRecurrence : RecurrenceRule {
    override fun occurrences(pattern: MaterializationRecurrencePattern, endInclusive: LocalDate?): List<LocalDate> {
        val result = mutableListOf<LocalDate>()
...
private class YearlyRecurrence : RecurrenceRule {
    override fun occurrences(pattern: MaterializationRecurrencePattern, endInclusive: LocalDate?): List<LocalDate> {
        val result = mutableListOf<LocalDate>()
...
private fun monthAllowed(month: YearMonth, pattern: MaterializationRecurrencePattern): Boolean =
    pattern.monthsOfYear?.contains(month.monthValue) ?: true

private fun candidatesForMonth(pattern: MaterializationRecurrencePattern, month: YearMonth): List<LocalDate> {
