...
class R2dbcPocketTransactionFeedRepository(
    ...
    override suspend fun findPage(pocketId: String, before: Long?, limit: Int): List<PocketTransactionFeedItem> {
        val sql = buildString {
            append(
                """
                SELECT pocket_id, account_id, contract_id, transaction_id, history_position, transaction_date, type, status, description,
    ...
    suspend fun findPageByContractId(contractId: String, before: Long?, limit: Int): List<PocketTransactionFeedItem> {
        val sql = buildString {
            append(
                """
                SELECT pocket_id, account_id, contract_id, transaction_id, history_position, transaction_date, type, status, description,
            ...
            if (before != null) append("\n  AND history_position < :before")
            append("\nORDER BY history_position DESC\nLIMIT :limit")
        }
        var spec = databaseClient.sql(sql).bind("contractId", contractId).bind("limit", limit)
        if (before != null) spec = spec.bind("before", before)
        return spec.fetch().all().let { Flux.from(it).map(::toPocketTransactionFeedItem).collectList().awaitSingle() }
    }
