...
class ModifyRecurringOccurrenceTest {
    ...
    fun `modifying recurring occurrence hides original and creates visible child`() = runTest {
        ...
        val modified = useCase.modifyRecurringOccurrence(
            TransactionService.ModifyRecurringOccurrenceCommand(
    ...
    fun `rejects modifying non recurring materialized rows or repeated chains`() = runTest {
        ...
        assertFailsWith<ValidationException> {
            useCase.modifyRecurringOccurrence(command("txn_already_modified"))
        }
        assertFailsWith<ValidationException> {
            useCase.modifyRecurringOccurrence(command("txn_manual"))
        }
    }

    private suspend fun useCase(transactionRepository: InMemoryTransactionRepository): TransactionService {
    ...
    private fun command(id: String) = TransactionService.ModifyRecurringOccurrenceCommand(
        transactionId = id,
        type = "EXPENSE",
        status = "CLEARED",
        transactionDate = "2024-04-12",
